...
`本扩展不执行远程代码。所有 JavaScript、HTML、CSS 和图片资源都打包在扩展安装包内。扩展只会在用户主动使用翻译、词典、OCR 或语法提示功能时，通过 HTTPS 请求第三方服务接口并处理返回的文本或 JSON 数据；这些返回内容不会作为代码执行。`

### storage

`用于在浏览器本地保存用户设置，包括源语言和目标语言、TTS 朗读参数、翻译源启用状态、网站白名单/黑名单，以及用户自行填写的第三方翻译服务配置。没有 storage 权限，扩展无法在浏览器重启后保留这些偏好设置。`

### contextMenus

`用于在网页右键菜单中提供“翻译选中文本”和“翻译图片文字”入口。该权限只用于用户主动点击右键菜单后触发翻译或 OCR 翻译，不会在后台自动读取网页内容。`

### scripting

`用于在用户通过右键菜单触发翻译时，确保页面侧内容脚本已注入并能接收翻译请求。该权限用于把译文卡片插入当前网页、协调页面内翻译 UI，以及在扩展重新加载后补充注入内容脚本。`

### host_permissions

`用于在用户浏览的网页上提供划词翻译、页面内插入译文、TTS 朗读入口、图片 OCR 右键翻译，以及访问用户启用的翻译、词典、OCR 和语法服务接口。扩展只在用户选中文本、点击右键菜单或使用相关功能时处理页面内容。`
